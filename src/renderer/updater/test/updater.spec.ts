...
describe('Updater Functions', () => {
        ...
        it('should handle status text updates', () => {
            const getStatusText = (state: string, releaseName?: string) => {
                switch (state) {
                    ...
                    case 'available':
                        return `Update to ${releaseName} Available!`;
                    case 'downloading':
            ...
            expect(getStatusText('available', 'v2.1.0')).toBe(
                'Update to v2.1.0 Available!'
            );
    ...
    describe('Release Notes Handling', () => {
        it('should process HTML release notes', () => {
            ...
            const htmlNotes =
                '<h3>Version 2.0.0</h3><ul><li>Feature A</li><li>Bug fix B</li></ul>';
    ...
    describe('Accessibility', () => {
        ...
        it('should provide meaningful status updates for screen readers', () => {
            ...
            expect(getAriaLabel('downloading', 50)).toBe(
                'Downloading update, 50% complete'
            );
            expect(getAriaLabel('ready')).toBe(
                'Update download complete, ready to restart'
            );
            expect(getAriaLabel('available')).toBe(
