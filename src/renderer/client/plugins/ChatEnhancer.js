...
var ChatEnhancer = class extends Plugin {
  constructor() {
    super();
    this.pluginName = "Chat Enhancer";
    this.author = "JayArrowz & Answerth";
    //TODO: Port CSS to a file inside resources
    this.observers = [];
    this.listeners = [];
    this.injectedEls = [];
    ...
    this.FILTERS = [
      ...
      { icon: "\u{1F535}", key: "private", section: "#hs-private-message-list" },
      { icon: "\u{1F7E0}", key: "global", className: "hs-text--orange" },
      { icon: "\u{1F7E1}", key: "local", className: "hs-text--yellow" },
      { icon: "\u26AA", key: "status", className: "hs-text--white" },
      { icon: "\u{1F3C1}", key: "opacity" }
    ];
    this.active = {
      chat: true,
      private: true,
  ...
  initializePlugin() {
    ...
    this.active.opacityEnabled = this.settings.opacityEnabled?.value;
    this.active.opacityValue = (this.settings.opacityValue.value || 60) / 100;
    this.applyStyles();
    this.applyFilters();
  ...
  applyStyles() {
    ...
    document.querySelectorAll(".hs-text--white").forEach((el) => {
      el.style.setProperty(
        "color",
        this.settings.statusMessageColor.value,
        "important"
      );
    });
    document.querySelectorAll(".hs-text--orange").forEach((el) => {
      el.style.setProperty(
        "color",
        this.settings.globalMessageColor.value,
        "important"
      );
    });
    document.querySelectorAll(".hs-text--yellow").forEach((el) => {
      el.style.setProperty(
        "color",
        this.settings.localMessageColor.value,
        "important"
      );
    });
    const container = document.getElementById(
    ...
    if (container) {
      if (this.settings.opacityEnabled?.value) {
        const opacity = this.settings.opacityValue.value / 100;
  ...
  applyChatDimensions() {
    ...
    const menu = document.querySelector("#hs-chat-menu");
    const input = document.querySelector(
      "#hs-chat-input-menu"
    );
    if (!pub || !pm || !menu || !input) return;
    const chatWidth = this.settings.chatWidth.value;
    const pubHeight = this.settings.publicChatHeight.value;
    const pmHeight = this.settings.privateChatHeight.value;
    ...
    if (chatMenu && chatInput) {
      ...
      const nameW = chatInput.previousElementSibling?.offsetWidth || 0;
  ...
  applyFilters() {
    ...
    this.active.chat = this.settings.showChatFilter.value;
    this.active.private = this.settings.showPrivateFilter.value;
    this.active.global = this.settings.showGlobalFilter.value;
    this.active.local = this.settings.showLocalFilter.value;
    this.active.status = this.settings.showStatusFilter.value;
    const sections = [
      {
  ...
  injectIntoSettingsMenu(menu) {
    if (menu.dataset.filtersInjected || !this.settings.enableFilters?.value || !this.settings.enable?.value || !this.isInitialized)
      return;
    ...
    let opacityToggleButton, opacitySlider;
      ...
      if (cfg.key === "opacity") {
        ...
        document.highlite.managers.UIManager.bindOnClickBlockHsMask(
          ...
          () => {
            ...
            button.style.outline = this.active[key] ? "2px solid deepskyblue" : "none";
            if (key === "chat")
              this.settings.showChatFilter.value = this.active.chat;
            if (key === "private")
              this.settings.showPrivateFilter.value = this.active.private;
            if (key === "global")
              this.settings.showGlobalFilter.value = this.active.global;
            if (key === "local")
              this.settings.showLocalFilter.value = this.active.local;
            if (key === "status")
              this.settings.showStatusFilter.value = this.active.status;
            document.highlite.managers.SettingsManager.updatePluginSettingsUI(
              this
            );
            ...
            if (sections.length) {
              sections.forEach((selector) => {
                document.querySelectorAll(selector).forEach((el) => {
                  el.style.display = this.active[key] ? "" : "none";
                });
              });
              if (this.active[key]) {
                sections.forEach((selector) => {
              ...
              document.querySelectorAll(
                ...
                if (msg.querySelector(
                  ...
                  msg.style.display = this.active[key] ? "" : "none";
                }
              });
            }
            if (cfg.key === "chat") {
  ...
  processNewMessages(container) {
    ...
    messages.forEach((msg) => {
      ...
      if (playerName && this.settings.extendedBlockList.value.split(",").some((name) => name.toUpperCase() === playerName?.toUpperCase())) {
      ...
      if (this.settings.enableFilters?.value) {
        this.FILTERS.forEach((filter) => {
          if (!filter.section && filter.key !== "opacity" && !this.active[filter.key]) {
  ...
  cleanupRemovedMessages(removedNodes) {
    removedNodes.forEach((node) => {
      if (node instanceof HTMLElement) {
        ...
        injectedElements.forEach((el) => {
          const index = this.injectedEls.indexOf(el);
          if (index > -1) {
  ...
  makeVerticalGrip(container, onResize, blocker) {
    ...
    let startY = 0;
    let startHeight = 0;
    this.trackListener(grip, "mousedown", (e) => {
      const mouseEvent = e;
      resizing = true;
...
var ChatEnhancer_default = ChatEnhancer;
export {
  ChatEnhancer,
  ChatEnhancer_default as default
};
